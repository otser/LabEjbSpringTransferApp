...
public class MoneyTransferServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (StringUtils.isEmpty(request.getParameter("facade")) || StringUtils.isEmpty(request.getParameter("amount"))) {
			response.getWriter().println("facade and amount are required");
			return;
		}

		MoneyTransferFacade moneyTransferFacade = moneyTransferFacadeLocator.locate(request.getParameter("facade"));
		if (moneyTransferFacade == null) {
			response.getWriter().println("Facade: " + moneyTransferFacadeLocator.getFacadesNames() + "?");
			return;
		}
		...
		MoneyTransferResponse transferResponse = moneyTransferFacade.transfer(new MoneyTransferRequestImpl(amount));
