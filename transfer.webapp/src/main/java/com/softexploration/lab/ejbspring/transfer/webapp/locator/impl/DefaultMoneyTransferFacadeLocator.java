...
public class DefaultMoneyTransferFacadeLocator implements MoneyTransferFacadeLocator {
	...
	private final List<String> facadesNames = Arrays.asList(new String[] { "ejb-spring", "spring-slejb", "spring-sfejb", "spring-singletonejb" });
		...
		if (facadesNames.contains(finalFacadeName)) {
			if ("ejb-spring".equals(finalFacadeName)) {
				try {
					...
					MoneyTransferFacade ret = (MoneyTransferFacade) ctx.lookup("java:comp/env/ejb/EJBMoneyTransferFacade");
					return ret;
	...
	private MoneyTransferFacade locateSpringMoneyTransferFacade(String beanName) {
		return WebApplicationContextUtils.getWebApplicationContext(servletContext).getBean(beanName,MoneyTransferFacade.class);
	}
